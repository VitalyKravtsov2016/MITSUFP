...
// ditron@dnc.dn.ua     //
//////////////////////////
//исправлена работа с менеджером памяти для x64
//исправлено получение значений параметров по умолчанию при регистрации метода
//добавлены методы преобразования переменных Int64, UInt64 и прочие...

interface

uses
  // VCL
  SysUtils, Windows, System.AnsiStrings,
  // This
  LogFile;
...
type
  ...
  public
    procedure Destroy1; virtual; stdcall; abstract;
...
type
  ...
  public
    procedure Destroy1; virtual; stdcall; abstract;
    function AddError(wcode: word; const source: PWideChar; const descr: PWideChar; scode: integer): boolean; virtual; stdcall; abstract;
    ...
    function GetPlatformInfo(): TAppInfo;  virtual; stdcall; abstract;
    function Alert(queryText: PWideChar): boolean;  virtual; stdcall; abstract;
  end;
...
type
  ...
  public
    ...
    procedure V8SetUInt64(V: PV8Variant; Value: UInt64);
...
function V8AsVariant(V: PV8Variant): Variant;
...
begin
  Result := System.AnsiStrings.strlen(V);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      pMemory^ := SysUtils.StrCat(pMemory^, #0);
      result := True;
    end;
  end;
end;
...
begin
  ...
  Result := SysUtils.StrCat(ClassNamesStringPWC, #0);
end;
...
begin
  ...
  for i := 0 to Obj.RelObj1.ClassReg.MethCount - 1 do
    if (_WideSameStr(Obj.RelObj1.ClassReg.MethList[i].MethNameLoc, wsMethodName)) or
        (_WideSameStr(Obj.RelObj1.ClassReg.MethList[i].MethName, wsMethodName)) then begin
      result := i;
      globallogger.Debug('_FindMethod: ' + Result.ToString);
      Break;
    end;
  globallogger.Debug('_FindMethod: ' + Result.ToString);
end;
...
begin
  ...
  for i := 0 to Obj.RelObj1.ClassReg.PropCount - 1 do
    if (_WideSameStr(Obj.RelObj1.ClassReg.PropList[i].PropNameLoc, wsPropName))
        or (_WideSameStr(Obj.RelObj1.ClassReg.PropList[i].PropName, wsPropName)) then begin
      result := i;
      Break;
    end;
end;
...
begin
  ...
  if Obj.RelObj1.ClassReg.MethList[lMethodNum].DefParams <> nil then
    for ind := 0 to Length(Obj.RelObj1.ClassReg.MethList[lMethodNum].DefParams.Values)-1 do
      if (Obj.RelObj1.ClassReg.MethList[lMethodNum].DefParams.Values[ind].Num = lParamNum) then begin
...
function _setMemManager(Obj: PV8ObjectRec; mem: TV8MemoryManager): boolean; stdcall;
begin
...
begin
  result := AddMethod(MethName, MethNameLoc, False, pointer(ExecuteProc), ParamCount);
end;
...
begin
  ...
  for i := 0 to MethCount - 1 do begin
    MethList[i].Free;
    MethList[i] := nil;
  end;
...
begin
  ...
  if Result then begin
    ...
    V.VarEnum.vtRecWideString.pwstrVal := SysUtils.StrCat(V.VarEnum.vtRecWideString.pwstrVal, #0);
...
procedure TV8UserObject.V8SetUInt64(V: PV8Variant; Value: UInt64);
begin
  V8ClearVar(V);
  V.vt := VTYPE_R8;
  V.VarEnum.dblVal := Value;
...
begin
  result := EncodeDate(UT.tm_year + 1900, UT.tm_mon + 1, UT.tm_mday) + EncodeTime(UT.tm_hour, UT.tm_min, UT.tm_sec, 0);
end;
...
begin
  ...
  begin
    if (V.VarEnum.intVal >= 0) then result := V.VarEnum.intVal else result := 0;
  end else
...
function V8AsVariant(V: PV8Variant): Variant;
begin
//  GlobalLogger.Debug(IntToStr(V.vt and VTYPE_TYPEMASK));

  case (V.vt and VTYPE_TYPEMASK) of
    VTYPE_PSTR, VTYPE_PWSTR: Result := V8AsWString(V);
    VTYPE_I4: Result := V8AsInt(V);
    VTYPE_R8: Result := V8AsDouble(V);
    VTYPE_BOOL: Result := V8AsBool(V);
  else
    result := '';
  end;
end;
...
initialization
  ...
  FillChar(_ProcRec, SizeOf(TV8ProcRec), 0);
  _ProcRec.Init := @_Init;
