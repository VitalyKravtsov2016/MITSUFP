...
{ COM server abstract base class }
  ...
  public
    constructor Create;
    procedure GetRegRootAndPrefix(var RootKey: HKEY; var RootPrefix: string);
    ...
    property PerUserRegistration: Boolean read FPerUserRegistration write FPerUserRegistration;
  end;
...
begin
  ...
  if (CoInitFlags <> -1) and Assigned(ComObj2.CoInitializeEx) then
  begin
    NeedToUninitialize := Succeeded(ComObj2.CoInitializeEx(nil, CoInitFlags));
...
constructor TComServerObject.Create;
begin
  inherited;
  PerUserRegistration := False;
end;

procedure TComServerObject.GetRegRootAndPrefix(var RootKey: HKEY;
                                                     var RootPrefix: string);
begin
  if (PerUserRegistration) then
  begin
