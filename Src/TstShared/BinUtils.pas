...
begin
  ...
  while True do
  begin
    Result := Result + Chr(IntToByteBCD(AValue mod 100));
...
begin
  ...
  Insert(DecimalSeparator, Result, Length(Result) - ADecimalPoint + 1);
end;
...
begin
  ...
  begin
    if i < 2 then Break;
    d := S[i - 1] + S[i];
    Result := Result + Chr(StrToBCDByte(d));
    Dec(i, 2);
  end;
end;
...
begin
  ...
  Result := Chr(Abs(y - 2000)) + Chr(m) + Chr(d) + Chr(h) + Chr(min);
end;
