...
begin
  ...
  try
    Tokens.Decode(Barcode);
    Token := Tokens.ItemByID('01');
    if Token = nil then
      raise Exception.CreateFmt(STagNotFound, ['GTIN(01)']);
    Result.GTIN := Token.Data;

    Token := Tokens.ItemByID('21');
    if Token = nil then
      raise Exception.CreateFmt(STagNotFound, ['SerialNumber(21)']);
...
const
  AIItems: array [0 .. 155] of TAIREc = ((id: '00'; min: 18; max: 18),
    ...
    (id: '7023'; min: 1; max: 30),
    // Global Individual Asset Identifier (GIAI) of an assembly
    (id: '7023'; min: 1; max: 30),
    // Global Individual Asset Identifier (GIAI) of an assembly
    (id: '703'; min: 4; max: 30), // Number of processor with ISO Country Code
    (id: '710'; min: 1; max: 20),
    // National Healthcare Reimbursement Number (NHRN) Ц Germany PZN
    (id: '711'; min: 1; max: 20),
    // National Healthcare Reimbursement Number (NHRN) Ц France CIP
    (id: '712'; min: 1; max: 20),
    // National Healthcare Reimbursement Number (NHRN) Ц Spain CN
    (id: '713'; min: 1; max: 20),
    // National Healthcare Reimbursement Number (NHRN) Ц Brasil DRN
    (id: '8001'; min: 14; max: 14),
...
procedure TGS1Tokens.Decode(const Data: AnsiString);
var
  i: Integer;
  Token: TGS1Token;
  State: TDecodeState;
