...
function AnsiToUnicode(const Text: string; CodePage: Word): WideString;
...
begin
  Result := '';
  if Text = '' then Exit;
  Len := MultiByteToWideChar(CodePage, MB_PRECOMPOSED, PChar(@Text[1]), -1, nil,
    0);
  SetLength(Result, Len - 1);
  if Len > 1 then
    MultiByteToWideChar(CodePage, MB_PRECOMPOSED, PChar(@Text[1]),
