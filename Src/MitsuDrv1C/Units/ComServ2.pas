...
uses Windows, Messages, ActiveX, SysUtils, ComObj2, TntWideStrUtils;
...
procedure UnregisterTypeLibrary(TypeLib: ITypeLib; PerUser: Boolean = False);
type
...
begin
  ...
  if WStrIComp(pszCmdLine, PWideChar(WideString('user'))) = 0 then
...
begin
  ...
  begin
    ...
    if (not IsInProcServer) and (StartMode = smAutomation)
      and Assigned(ComObj2.CoAddRefServerProcess) then
      ComObj2.CoAddRefServerProcess;
  end
  else
  begin
    Result := InterlockedDecrement(FObjectCount);
    if (not IsInProcServer) and (StartMode = smAutomation)
      and Assigned(ComObj2.CoReleaseServerProcess) then
    begin
      if ComObj2.CoReleaseServerProcess = 0 then
        LastReleased;
    end
...
begin
  ...
  ComServer.FStartSuspended := (CoInitFlags <> -1) and
    Assigned(ComObj2.CoInitializeEx) and Assigned(ComObj2.CoResumeClassObjects);
  ComServer.Initialize;
  if ComServer.FStartSuspended then
    ComObj2.CoResumeClassObjects;
end;
