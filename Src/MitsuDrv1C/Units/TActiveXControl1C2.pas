...
uses
  ...
  // This
  ComObj, AddIn1CTypes, AddIn1CInterface, untLogger, {DriverTypes,} LogFile, AxCtrls,
  acxmps;

type
  { TActiveXControl1C }

  TActiveXControl1C2 = class(TActiveXComponent, IInitDone, ILanguageExtender)
  private
    FLogger: TLogger;
    FProps: TAddinProps;
...
destructor TActiveXControl1C2.Destroy;
begin
...
procedure TActiveXControl1C2.UpdateAddinLists;
var
  I, J, k: Integer;
  Prop: TAddinProp;
  Func: TAddinFunc;
...
function TActiveXControl1C2.RegisterExtensionAs(var ExtensionName: WideString): HResult;
begin
  ExtensionName := Factory.ClassName;
  Result := S_OK;
end;

procedure TActiveXControl1C2.PutNParam(var pArray: PSafeArray; lIndex: Integer; var varPut: OleVariant);
begin
  SafeArrayPutElement(pArray,lIndex,varPut);
end;

function TActiveXControl1C2.GetNParam(var pArray : PSafeArray; lIndex: Integer ): OleVariant;
var
  varGet : OleVariant;
begin
...
function TActiveXControl1C2.vtToStr(vt: Word):string;
begin
  case vt and $FF of
...
function TActiveXControl1C2.CallAsFunc(Index: Integer; var RetValue: OleVariant;
...
var
  ...
  S: string;
begin
  Logger.Debug(Format('%s(%d)', ['TActiveXControl1C2.CallAsFunc', Index]));
  pParams := nil;
  cElements := 0;
...
function TActiveXControl1C2.CallAsProc(Index: Integer; var Params: PSafeArray): HResult;
var
  RetValue: OleVariant;
begin
  RetValue := 0;
  Result := CallAsFunc(Index, RetValue, Params);
  Logger.Debug(Format('TActiveXControl1C2.CallAsProc(%d)=%d', [Index, Result]));
end;

function TActiveXControl1C2.FindMethod(const MethodName: WideString;
...
function TActiveXControl1C2.FindProp(const PropName: WideString;
  var Index: Integer): HResult;
var
  i: Integer;
  S: WideString;
...
function TActiveXControl1C2.GetMethodName(Index, Alias: Integer;
...
begin
  ...
  Logger.Debug(Format('TActiveXControl1C2.GetMethodName(%d,%d,%s)=%d',
    [Index, Alias, MethodName, Result]));
end;

function TActiveXControl1C2.GetNMethods(var Count: Integer): HResult;
begin
  Logger.Debug('TActiveXControl1C2.GetNMethods');

  Count := Methods.Count;
  Result := S_OK;
  Logger.Debug(Format('TActiveXControl1C2.GetNMethods(%d)=%d', [Count, Result]));
end;
...
function TActiveXControl1C2.GetNParams(Index: Integer; var Count: Integer): HResult;
begin
  Logger.Debug('TActiveXControl1C2.GetNParams');
  Result := S_FALSE;
  if (Index >= 0) and (Index < Methods.Count) then
  begin
  ...
  Logger.Debug(Format('TActiveXControl1C2.GetNParams(%d, %d)=%d', [Index, Count, Result]));
end;

{ Количество свойств }

function TActiveXControl1C2.GetNProps(var Count: Integer): HResult;
begin
  Logger.Debug('TActiveXControl1C2.GetNProps');

  Count := Props.Count;
  Result := S_OK;
  Logger.Debug(Format('TActiveXControl1C2.GetNProps(%d)=%d', [Count, Result]));
end;

function TActiveXControl1C2.GetParamDefValue(mIndex, pIndex: Integer;
  var ParamDefValue: OleVariant): HResult;
var
  Func: TAddinFunc;
  //Param: TAddinParam;
begin
  Logger.Debug('TActiveXControl1C2.GetParamDefValue');
...
function TActiveXControl1C2.GetPropName(Index, Alias: Integer;
  var PropName: WideString): HResult;
begin
  Logger.Debug('TActiveXControl1C2.GetPropName');
...
function TActiveXControl1C2.GetPropVal(Index: Integer; var Value: OleVariant): HResult;
var
  DispParams: TDispParams;
begin
  Logger.Debug('TActiveXControl1C2.GetPropVal');
...
function TActiveXControl1C2.HasRetVal(Index: Integer; var RetValue: Integer): HResult;
begin
  Logger.Debug('TActiveXControl1C2.HasRetVal');
...
function TActiveXControl1C2.IsPropReadable(Index: Integer; var Value: Integer): HResult;
begin
  Logger.Debug('TActiveXControl1C2.IsPropReadable');
  Result := S_FALSE;
  if (Index >= 0) and (Index < Props.Count) then
  begin
...
function TActiveXControl1C2.IsPropWritable(Index: Integer; var Value: Integer): HResult;
begin
  Logger.Debug('TActiveXControl1C2.IsPropWritable');
  Result := S_FALSE;
  if (Index >= 0) and (Index < Props.Count) then
  begin
...
function TActiveXControl1C2.SetPropVal(Index: Integer; var Value: OleVariant): HResult;
var
  DispParams: TDispParams;
begin
  Logger.Debug('TActiveXControl1C2.SetPropVal');
  with DispParams do
  begin
    rgvarg := @Value;
...
function TActiveXControl1C2.GetLogger: TLogger;
begin
  if FLogger = nil then
  begin
...
function TActiveXControl1C2.Init(pConnection: IDispatch): HResult;
begin
  Result := S_OK;
end;

function TActiveXControl1C2.Done: HResult;
begin
  Result := S_OK;
end;

function TActiveXControl1C2.GetInfo(var pInfo: PSafeArray): HResult;
var
  Index: Integer;
  Value: OleVariant;
begin
...
procedure TActiveXControl1C2.Initialize;
begin
...
function TActiveXControl1C2.SetLocale(bstrLocale: WideString): HResult;
begin
  Result := S_OK;
  SetLanguage(bstrLocale);
end;

procedure TActiveXControl1C2.SetLanguage(LangType: string);
begin

end;

procedure TActiveXControl1C2.LogMethods;
var
  i: Integer;
  FP: TStringList;
  FM: TStringList;
